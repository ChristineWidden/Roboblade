...
function getChannel(auth) {
    ...
    service.playlistItems.list({
        ...
        if (info.length == 0) {
            ...
            videoAmount = response.data.pageInfo.totalResults;

            console.log("Video amount: " + videoAmount);

            randomVideoNumber = Math.floor(Math.random() * videoAmount);

            console.log("Random video number: " + randomVideoNumber);

            nextPageToken = response.data.nextPageToken;

            getPage(service, auth, nextPageToken, randomVideoNumber)
...
async function getPage(service, auth, nextPageToken, randomVideoNumber) {
    ...
    pageNeeded = Math.floor(randomVideoNumber/50);
    videoToFind = randomVideoNumber % 50;
    ...
    if(pageNeeded < 1) {
        getNiceVideo(service, auth, "", videoToFind);
    } else {

        for (let i = 0; i < pageNeeded-1; i++) {
            token = await getNextToken(service, auth, token);
            console.log("Retrieved: " + token);
        }

        getNiceVideo(service, auth, token, randomVideoNumber%50)
    }
}
...
async function getNiceVideo(service, auth, token, itemId) {
    service.playlistItems.list({
        auth: auth,
        part: "snippet,status",
        playlistId: "UUFAiFyGs6oDiF1Nf-rRJpZA",
